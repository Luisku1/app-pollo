...
import { useDeleteEmployeeRest } from '../hooks/Employees/useDeleteEmployeeRest'
import DeleteButton from './Buttons/DeleteButton'

export default function RestsList({ rests, spliceEmployeeRest }) {
    ...
    <div>
      ...
      {rests && rests.length != 0 && rests.map((rest, index) => (
        ...
        <div key={rest._id} className='grid grid-cols-12 mt-4 border border-black rounded-lg shadow-lg p-3 gap-2'>
          ...
          <div>
            <DeleteButton id={rest._id} deleteFunction={deleteEmployeeRest} index={index} item={rest} spliceFunction={spliceEmployeeRest}></DeleteButton>
          </div>
        </div>
      ))}
    </div>
