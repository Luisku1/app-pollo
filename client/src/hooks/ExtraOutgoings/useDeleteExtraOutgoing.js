...
export const useDeleteExtraOutgoing = () => {
  ...
  const deleteExtraOutgoing = async (extraOutgoing) => {
    ...
    try {
      ToastSuccess(`Se borró el gasto fuera de cuentas de ${currency({ amount: extraOutgoing.amount })}`);

      await deleteExtraOutgoingFetch({ extraOutgoingId: extraOutgoing._id });
    } catch (error) {
      console.error(error);
      ToastDanger(`No se borró el gasto fuera de cuentas de ${currency({ amount: extraOutgoing.amount })}`);
    } finally {
      setLoading(false);
    }
  };
